...
export async function sendQuestion(questionData: NpcQuestionData | string) {
  ...
  //const chatMessage: serverStateSpec.ChatMessage = createMessageObject(response, undefined, GAME_STATE.gameRoom)
  //console.log("Nuovo pacchetto creato con la risposta di Rasa:", chatMessage);
  //sendMsgToAI(chatMessage)
  //talk(REGISTRY.activeNPC.entity, chatMessage);
  console.log("Active NPC:", REGISTRY.activeNPC);
  //const littleRobotNPC: serverStateSpec.CharacterId = { resourceName: "workspaces/genesis_city/characters/simone" }; // Da eliminare se non funziona
