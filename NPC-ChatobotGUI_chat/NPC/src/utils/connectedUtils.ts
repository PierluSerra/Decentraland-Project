...
export function createMessageObject(msgText: string, characterId: serverStateSpec.CharacterId, room: Room<clientState.NpcGameRoomState>) {
  ...
  console.log("ecco il messaggio", chatMessage)
  return chatMessage
}

//export function createMessageObject(
//   msgText: string,
//   characterId: serverStateSpec.CharacterId | undefined,
//   room: Room<clientState.NpcGameRoomState>
// ) {
//   const uniqueId = new Date().getTime().toString(); // genera un ID unico
//   const chatMessage: serverStateSpec.ChatMessage = new serverStateSpec.ChatMessage({
//     date: new Date().toUTCString(),
//     packetId: { interactionId: uniqueId, packetId: uniqueId, utteranceId: uniqueId },
//     type: serverStateSpec.ChatPacketType.TEXT, // deve essere TEXT (1)
//     text: { text: msgText, final: true },         // inserisci la risposta di Rasa qui
//     routing: {
//       source: { isCharacter: false, isPlayer: true, name: room.sessionId, xId: { resourceName: room.sessionId } },
//       target: { isCharacter: true, isPlayer: false, name: "", xId: characterId ? characterId : lastCharacterId }
//     },
